...
public class ItemSlot extends Slot {
    ...
    public ItemSlot(ItemStack stack, int width, int height) {
        super(width, height);
        this.stack = stack;
    }
    ...
    public void draw(GraphicsHandler handler, int x, int y) {
        ...
        this.frameCount++;
        var frames = this.stack.getItem().getFrames().get("inventory");
        if (frames != null) {
        ...
        handler.drawStringWithOutline(Integer.toString(this.stack.getCount()), x + this.width - 8 - 5, y + this.height - 5, Resources.press_start.deriveFont(8f), TailwindColorScheme.white, TailwindColorScheme.slate900, 3);
    }
